...
export default function OrderSteps({ hasText }: { hasText?: boolean }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-col md:flex-row gap-2 items-center">
        ...
        <div className="flex flex-col items-center gap-2 p-2 border-2 border-stroke rounded-xl h-full">
          <CalendarMonthIcon />
          <p style={{ color: "white" }}>Gemeinsam wird ein Termin vereinbart</p>
        </div>
        <ArrowForwardOutlinedIcon />
        <div className="flex flex-col items-center gap-2 p-2 border-2 border-stroke rounded-xl h-full">
          <LocalShippingOutlinedIcon />
          <p style={{ color: "white" }}>Die Zustellung erfolgt termingerecht</p>
        </div>
      </div>
    </div>
  );
